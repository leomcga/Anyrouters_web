...
test('PowerShell installers use native JSON, full-catalog validation, backups, and atomic replacement', () => {
  for (const name of ['codex.ps1', 'codex-config.ps1']) {
    ...
    expect(script).toContain('$replaceBackup')
    expect(script).toContain('Remove-Item $replaceBackup -Force -ErrorAction SilentlyContinue')
    expect(script).not.toContain('[System.IO.File]::Replace($Source, $Destination, $null)')
